...
function TimerSlideNext() {
  CreateSlideMini(GetClassNumberNext());
  GetOffsetAnimation();
}
function GetClassNumberNext() {
...
function CreateSlideMini(classNumber) {
  let img = document.createElement("img");
...
function TimerSlideBack() {
  CreateSlideReverseMini(GetClassNumberBack());
  GetOffsetAnimationReverse();
}
function GetClassNumberBack() {
  let tempSlideHtml = document.querySelectorAll(".slide-single");
  let tempClassName = tempSlideHtml[0].className;
  let classNumber = parseInt(tempClassName.match(/\d+/));
...
function CreateSlideReverseMini(classNumber) {
  let img = document.createElement("img");
  img.src = srcSlide[classNumber];
  img.classList.add("slide-single");
  img.classList.add(`slide--${classNumber + 1}`);
...
function GetOffsetAnimationReverse() {
  let currentSlideInHtml = document.querySelectorAll(".slide-single");
  let currentPositionIndex1 = -530;
  let currentPositionIndex0 = 0;
  let stepPosition = 5;
...
function GetOffsetAnimationGeneric(stepPosition,currentPosition, stopIntervalPosition) {
    let currentSlideInHtml = document.querySelectorAll(".slide-single");
